...
$composeProject = 'kicad-routing-automation'
if ($Command -eq 'build') {
    & docker compose --project-directory $repoRoot -f (Join-Path $repoRoot 'compose.yaml') build kicad
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & docker compose --project-name $composeProject --project-directory $repoRoot -f $compose build routing
} else {
    ...
    & docker compose --project-name $composeProject --project-directory $repoRoot -f $compose @routingArgs
}
exit $LASTEXITCODE
